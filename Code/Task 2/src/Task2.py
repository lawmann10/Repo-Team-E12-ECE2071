...
def save_files(fileType, data):
    if fileType.lower == "wav":
        ...
        plt.savefig(f"./outputs/E12_{SAMPLE_RATE}Hz_image.png")

    elif fileType.lower == "csv":

        with open(f"./outputs/E12_{SAMPLE_RATE}Hz_data.csv", "w") as file:
            file.write(f"{SAMPLE_RATE}\n")
            file.write(data)

def distance_STM(val1, val2):
    distance  = (val1 - val2 * 0.034/2)
    return distance
