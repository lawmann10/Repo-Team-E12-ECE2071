...
def distance_mode():
    ...
    try:
        while True:
            ...
            # Read audio using escape protocol:
            #   0xFF 0xFF = end of recording
            #   0xFF 0x00 = audio sample with value 0xFF
            #   anything else = raw audio byte
            escape_next = False
            while True:
                byte = ser.read(1)
                if not byte:
                ...
                if escape_next:
                    escape_next = False
                    if b == 0xFF:       # stop signal
                        break
                    elif b == 0x00:     # escaped 0xFF audio sample
                        audio.append(0xFF)
                elif b == 0xFF:
                    escape_next = True
                else:
